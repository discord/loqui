defmodule Loqui.Client do
  ...
  def connect(
        _info,
        %{sock: nil, host: host, port: port, tcp_opts: tcp_opts, connect_timeout: connect_timeout} =
          state
      ) do
  ...
  def handle_call(
        {:request, payload},
        caller,
        %State{codec: codec, compressor: compressor} = state
      ) do
    {next_seq, state} = State.next_sequence(state)
  ...
  def handle_info(
        {:close_go_away, go_away_code, go_away_data},
        %State{sequence: :go_away, waiters: waiters} = state
      ) do
  ...
  defp handle_packet(
         {:response, _flags, sequence, payload},
         %State{codec: codec, compressor: compressor} = state
       ) do
  ...
  defp do_loqui_connect(
         %State{sock: sock, registered_codecs: codecs, registered_compressors: compressors} =
           state
       ) do
    extract_names = fn m ->
      m
