...
static inline int drpc_append_hello(drpc_buffer_t *b, uint32_t ping_interval) {
  unsigned char buf[2 + sizeof(uint32_t)];
  buf[0] = DRPC_OP_HELLO;
  buf[1] = DRPC_VERSION;
  _drpc_store32(buf + 2, ping_interval);
  drpc_append(b, buf, 2 + sizeof(uint32_t));
}
...
static inline int drpc_append_ping(drpc_buffer_t *b, uint32_t seq) {
  ...
  drpc_append(b, buf, 1 + sizeof(uint32_t));
}
...
static inline int drpc_append_request(drpc_buffer_t *b, uint32_t seq, uint32_t size, const char* data) {
  #define SIZE 1 + sizeof(uint32_t) + sizeof(uint32_t)
...
static inline int drpc_append_goaway(drpc_buffer_t *b, uint8_t code, uint32_t size, const char* data) {
  #define SIZE 1 + 1 + sizeof(uint32_t)
  unsigned char buf[SIZE];
